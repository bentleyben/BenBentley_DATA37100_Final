...
try:
    ...
    raise ImportError(
        "diffusion_baseline.py requires torchvision. Install with: pip install torchvision"
    ) from e
...
def _dataset_exists(root: Path, dataset: str) -> bool:
    if dataset == "mnist":
        return (root / "MNIST" / "raw").exists() or (
            root / "MNIST" / "processed"
        ).exists()
    return (root / "FashionMNIST" / "raw").exists() or (
        root / "FashionMNIST" / "processed"
    ).exists()


def get_local_dataloader(
    dataset: str, data_dir: str, bs: int, num_workers: int, download: bool
    ...
    if dataset == "mnist":
        ds = datasets.MNIST(
            root=str(root), train=True, download=download, transform=tfm
        )
    else:
    ...
    if platform.system() == "Darwin" and num_workers > 0:
        print(
            f"[DataLoader] macOS detected → forcing num_workers=0 (was {num_workers})"
        )
        num_workers = 0
    ...
    return DataLoader(
        ds, batch_size=bs, shuffle=True, num_workers=num_workers, drop_last=True
    )
...
def run_single(args: argparse.Namespace, overrides: Dict[str, str]) -> Path:
    ...
    dl = get_local_dataloader(
        dataset=a.dataset,
        data_dir=a.data_dir,
        bs=a.bs,
        num_workers=a.num_workers,
        download=getattr(a, "download", False),
    )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Final Project Starter — Diffusion Baseline"
    )
    ...
    p.add_argument(
        "--download",
        action="store_true",
        help="Download dataset if missing (default: off)",
    )
    ...
    p.add_argument(
        "--grid", type=str, default="", help='Example: "T=100,200,400;target=eps,x0"'
    )
    p.add_argument(
        "--max-runs", type=int, default=6, help="Hard cap on number of runs from --grid"
    )
